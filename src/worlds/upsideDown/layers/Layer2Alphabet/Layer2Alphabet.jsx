...
export default function Layer2Alphabet({ onLetterClick, sessionId }) {
  const [clickedLetter, setClickedLetter] = React.useState(null)

  const handleClick = (letter) => {
    if (onLetterClick && sessionId) {
      onLetterClick(letter)
      
      // Visual feedback - glow for 800ms
  ...
  return (
    <div className="ud-layer ud-alphabet">
      {/* SVG filter for rough painted texture */}
      <svg style={{ position: 'absolute', width: 0, height: 0 }}>
        <defs>
          <filter id="roughPaintUD">
      ...
      <div className="ud-alphabetStage">
        {LETTERS.map(({ c, x, y, r, s }) => (
          <span
            key={c}
            className={`ud-letter ${sessionId ? 'ud-letter--clickable' : ''} ${clickedLetter === c ? 'ud-letter--clicked' : ''}`}
            style={{
              '--x': `${x}%`,
              '--y': `${y}%`,
              '--r': `${r}deg`,
